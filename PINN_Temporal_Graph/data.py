"""
Data processing utilities for circadian rhythm analysis
"""
...
class CircadianDataset:
    """
    Dataset class for circadian rhythm data
    """
    def __init__(self, expression_data, time_points, normalize=True):
        """
        Args:
            expression_data: (n_samples, n_genes) or (n_timepoints, n_eigengenes)
            time_points: (n_samples,) time values
        ...
        if normalize:
            # Z-score normalize each gene/eigengene
...
def load_expression_data(file_path, time_col='time', sep='\t'):
    ...
    Expected format:
    - Columns: time, gene1, gene2, ..., geneN
    ...
    if time_col in df.columns:
        ...
        # Assume first column is time if not specified
...
def create_time_points(n_points=24, period=24.0):
    """
    Create evenly spaced time points over a circadian period
    """
    return torch.linspace(0, period, n_points)

def interpolate_to_grid(expression_data, time_points, target_times):
    """
    Interpolate expression data to a regular time grid
    """
...
def detect_periodic_patterns(expression_data, time_points, min_period=20, max_period=28):
    """
    Detect periodic patterns in expression data using FFT
    """
    ...
    return np.mean(periods) if periods else 24.0  # Default to 24 hours
...
def prepare_training_data(data_path, n_eigengenes=5, n_time_points=100):
    """
    Prepare data for training from raw expression files
    """
    # Load data
...
def create_graph_from_eigengenes(eigengenes, method='correlation'):
    """
    Create initial graph structure from eigengene correlations
    """
    if method == 'correlation':
        ...
        # Mutual information-based (placeholder)
...
def validate_circadian_data(expression_data, time_points):
    """
    Validate circadian data quality
    """
    issues = []
