...
data_path = "/home/xuzhen/data"
path_to_cyclops = joinpath(base_path, "CYCLOPS.jl")
output_path = joinpath(base_path, "output")

expression_data = CSV.read(joinpath(data_path, "GSE54652", "brown_adipose", "expression.csv"), DataFrame)
expression_data[:, 1] = uppercase.(string.(expression_data[:, 1]))
seed_genes = expression_data[2:end, 1]
...
# 显式同步所有变量到 worker，防止未定义
for v in [:base_path, :data_path, :path_to_cyclops, :output_path]
    @eval Distributed.@everywhere $(v) = $(eval(v))
end
...
training_parameters[:align_genes] = uppercase.(string.(training_parameters[:align_genes]))
