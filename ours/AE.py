...
class PhaseAutoEncoder(nn.Module):
    def decoder_jacobian_norm(self, phase_angles, device=None):
        # Prepare scalar phase input and allow gradients
        phase = phase_angles
        if phase.dim() == 1:
            phase = phase.unsqueeze(1)
        if device is not None:
            phase = phase.to(device)
        phase = phase.requires_grad_(True)

        # If the decoder expects Fourier features, build them from scalar phase
        ...
        outputs = self.decoder(phase_input)  # (batch, input_dim)
        ...
        for i in range(batch):
            ...
            grad1 = torch.autograd.grad(
                outputs, phase, grad_outputs=grad_outputs,
                retain_graph=True, create_graph=True, allow_unused=True
            )[0]
            if grad1 is None:
                g = torch.tensor(0.0, device=outputs.device)
            else:
                g = torch.abs(grad1[i, 0])
            norms.append(g)

        return torch.stack(norms)
    
    def __init__(self, input_dim, n_celltypes=0, celltype_embedding_dim=4, dropout=0.1,
                 fourier_K=3, fourier_hidden=128, fourier_activation=nn.SiLU):
    ...
    def decode(self, phase_angles):
        """
        Accept phase_angles (radians) as (batch,) tensor, reshape to (batch, 1) and decode.
        """
